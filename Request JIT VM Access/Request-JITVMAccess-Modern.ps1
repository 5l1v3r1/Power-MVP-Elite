...
Author    : Original: Charbel Nemnom; Update: Alex Harmon
Version   : 3.0
Date      : 20-August-2018
...
Function Install-Az {
    Set-PSRepository -Name PSGallery -Installation Trusted -Verbose:$false
    Install-Module -Name Az -RequiredVersion 3.7.0 -Confirm:$false -Verbose:$false
}

Function Install-AzSecurity {
    Set-PSRepository -Name PSGallery -Installation Trusted -Verbose:$false
    Install-Module -Name Az.Security -AllowPrerelease -Confirm:$false -Verbose:$false
}
...
Function Enable-JITVMAccess {
    ...
    Set-AzJitNetworkAccessPolicy -VirtualMachine $JitPolicyArr -ResourceGroupName $VMInfo.ResourceGroupName -Location $VMInfo.Location -Name "default" -Kind "Basic" | Out-Null
}
...
Try {
    Import-Module -Name Az -RequiredVersion 3.7.0 -ErrorAction Stop -Verbose:$false | Out-Null
    Write-Verbose "Importing Az PowerShell Module..."
}
Catch {
    Write-Warning "Az Module requies update..."
    Write-Verbose "Installing the latest Az Module..."
    Install-Az
}
...
Catch {
    Write-Warning "Az Security PowerShell Module was not found..."
    Write-Verbose "Installing Az Security PowerShell Module..."
    Install-AzSecurity
}
...
$AzureSubscriptions = Get-AzSubscription | Where-Object {$_.Name -notlike "*Azure Active Directory*"}
$MaxSub = ($AzureSubscriptions.Count) - 1
$Sub = 0
do {
    Set-AzContext -SubscriptionId $AzureSubscriptions[$Sub].Id | Out-Null
